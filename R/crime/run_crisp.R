...
write.csv(y_hat, file.path(sprintf("../../data/crime/%s/predictions/crisp/%d.csv", city, trial)), row.names=FALSE)
...
write.csv(c(rmse,maxerr), file.path(sprintf("../../data/crime/%s/results/crisp/%d.csv", city, trial)), row.names=FALSE)

y_sweep = predict(model, X_sweep, best.lambda)
write.csv(y_sweep, file.path(sprintf("../../data/crime/%s/sweeps/crisp/%d.csv", city, trial)), row.names=FALSE)
