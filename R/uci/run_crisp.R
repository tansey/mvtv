...
N = dim(train_data)[1]
q = min(N, 100) # The number of splits per dimension, capped at 100
X = train_data[, c(1, 2)]
...
write.csv(y_hat, file.path(sprintf("../../data/uci/%s/predictions/crisp/%d.csv", dataset, trial)), row.names=FALSE)
...
write.csv(c(rmse, maxerr, q, lambda.seq[best.lambda]), file.path(sprintf("../../data/uci/%s/results/crisp/%d.csv", dataset, trial)), row.names=FALSE)

y_sweep = predict(model, X_sweep, best.lambda)
write.csv(y_sweep, file.path(sprintf("../../data/uci/%s/sweeps/crisp/%d.csv", dataset, trial)), row.names=FALSE)
